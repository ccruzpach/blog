...
<title>Sample Blog Using Laravel</title>
...
<body style="font-family: Open Sans, sans-serif">
    <section class="px-6 py-8">
        <nav class="md:flex md:justify-between md:items-center">
            ...
            <div>
                <a href="/" class="text-s font-bold uppercase ml-20">Home Page</a>
            </div>

            <div class="mt-8 md:mt-0 ml-auto flex items-center">

                @auth
                <span class="ml-4 text-xs font-bold"> Welcome, {{ auth()->user()->name }}</span>

                <form method="POST" action="/logout" class="ml-4 text-xs font-semibold text-blue-500">
                    @csrf
                    <button type="submit">Log Out</button>
                </form>
                @else
                <a href="/register" class="ml-4 text-xs font-bold uppercase">Register</a>
